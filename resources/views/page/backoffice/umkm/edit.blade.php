...
@section('title', 'Buat UMKM Baru')
@section('content')
<form method="POST" action="{{ url('backoffice/umkm/' .$umkm->id) }}">
@method('PUT')
    @csrf
    @if ($errors->any())
        ...
        <div class="col-12">
            <div class="card shadow mb-4">
                <div class="card-header py-3">
                    <h6 class="m-0 font-weight-bold text-primary">Ubah Data UMKM</h6>
                </div>
                <div class="card-body">
                    <div class="row">
                        <div class="col-sm-12">
                            <div class="form-group">
                                <label for="jenis_usaha">Pemilik UMKM</label>
                                <select class="form-control" name="user" id="form_user">
                                    <option selected disabled>Pilih Pemilik UMKM</option>
                                    @foreach($user as $u)
                                    <option value="{{ $u->id }}"  @if($umkm->id_user == $u->id) selected @endif>{{ $u->nama }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
